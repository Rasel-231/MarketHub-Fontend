      ...
      {ordersArray.map((order) => (
        <div key={order.id} className="max-w-4xl mx-auto p-4 space-y-8 mb-10">
          <div className="flex flex-col items-center gap-4">
            ...
            <h1 className="text-3xl font-bold font-nunito text-gray-800">
              Order Status
            </h1>
          </div>
          ...
          <div className="border rounded-xl shadow-lg bg-white overflow-hidden">
            <div className="bg-blue-50 p-4 border-b">
              <h1 className="font-bold font-nunito text-xl text-gray-800 italic">
                Order Summary
              </h1>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4 p-6">
              <div className="flex justify-between border-b pb-2">
                ...
                <p className="font-semibold text-gray-800">
                  {" "}
                  {`O-${order.id.slice(0, 6).toUpperCase()}`}
                </p>
              </div>

              <div className="flex justify-between border-b pb-2">
                <p className="text-gray-500 text-sm">Shipping Address</p>
                <p className="font-semibold text-gray-800 text-right max-w-[200px]">
              ...
              <div className="flex justify-between border-b pb-2">
                ...
                <span
                  className={`px-2 py-1 rounded text-xs font-bold ${order.status === "DELIVERED" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}
                >
                  {order.status}
                </span>
              </div>
