...
import { useGetRelatedProductsQuery } from "@/store/api/flagApi/flagApi";
import RelatedProductEffect from "./RelatedProdutctEffect";

const RelatedProduct = ({ currentProductId }: { currentProductId: string }) => {
  const { handleAdd } = useAddToCart();
  const { data: ProductResponse, isLoading, error } = useGetRelatedProductsQuery(currentProductId);
  const products = Array.isArray(ProductResponse?.data) 
    ? (ProductResponse.data as IUserProducts[]) 
    : [];
    console.log("Frontend Related Products",ProductResponse)

 if (isLoading) return <div className="flex justify-center p-10"><CustomSpinner/></div>;
 if (error) return <div className="text-center text-red-500 p-10">Related products cannot be loaded.</div>;
if (products.length === 0) return null;
  return (
    <div>
      <MinHeader title="Related Products" />
      <RelatedProductEffect>
        {products.map((product) => (
          <div
            key={product.id}
            className="border shadow-md bg-white relative m-2 rounded-sm flex flex-col h-[260px] sm:h-[380px] overflow-hidden w-48 sm:w-64 flex-shrink-0"
          >
            ...
            <div className="mt-2 px-3 flex flex-col justify-between">
              <div className="mb-3">
              ...
              <button
                ...
                className="w-full bg-black text-white py-1.5 sm:py-2 flex items-center justify-center gap-2 hover:bg-gray-800 transition-colors text-sm"
              >
