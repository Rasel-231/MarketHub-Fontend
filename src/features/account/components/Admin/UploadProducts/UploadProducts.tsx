...
const UploadProducts = () => {
  ...
  const { data: attrResponse, isFetching: isAttrLoading } =
    useGetAttributeQuery(category, {
      skip: !category,
  ...
  const dynamicAttributes: IAttribute[] =
    category && attrResponse?.success && Array.isArray(attrResponse?.data)
      ? attrResponse.data
      : [];
  ...
  const handleUploadProduct = async (e: React.FormEvent) => {
    ...
    if (!actualUserId || !actualSellerId)
      return toast.error("Seller profile not detected.");
        ...
        <form onSubmit={handleUploadProduct} className="p-8 space-y-8">
          <div className="space-y-6">
            <h3 className="flex items-center gap-2 font-bold text-gray-800 dark:text-gray-200 border-b pb-2">
              <Package size={18} className="text-emerald-600" /> Basic
              Information
            </h3>
          ...
          <div className="space-y-6">
            <h3 className="flex items-center gap-2 font-bold text-gray-800 dark:text-gray-200 border-b pb-2">
              <DollarSign size={18} className="text-emerald-600" /> Pricing &
              Inventory
            </h3>
          ...
          <div className="space-y-6">
            <h3 className="flex items-center gap-2 font-bold text-gray-800 dark:text-gray-200 border-b pb-2">
              <Layers size={18} className="text-emerald-600" /> Category &
              Attributes
            </h3>
                  ...
                  dynamicAttributes.map((attr) => (
                    ...
                    >
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          onChange={(e) =>
                            setTempInput({
                              ...tempInput,
                              [attr.label || attr.name]: e.target.value,
                            })
                          }
                          onKeyDown={(e) =>
                            e.key === "Enter" &&
                            (e.preventDefault(),
                            handleAttrAdd(attr.label || attr.name))
                          }
                        ...
                        <button
                          type="button"
                          onClick={() => handleAttrAdd(attr.label || attr.name)}
                      ...
                      <div className="flex flex-wrap gap-2 mt-3">
                        {(
                          selectedAttributes[attr.label || attr.name] || []
                        ).map((val, idx) => (
                          <span
                            key={idx}
                            className="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 text-[10px] font-black rounded-lg border border-emerald-200 dark:border-emerald-800"
                          >
                            {val}{" "}
                            <X
                              size={12}
                              ...
                              onClick={() =>
                                removeAttrValue(attr.label || attr.name, val)
                              }
                            />
                          </span>
                        ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <label className="border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-xl p-8 flex flex-col items-center justify-center cursor-pointer hover:bg-emerald-50 dark:hover:bg-emerald-950/20 transition-all">
                ...
                <p className="text-sm mt-3 font-medium text-gray-500">
                  {product_images.length > 0
                    ? `${product_images.length} image(s) selected`
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setProduct_Images(selectedFiles);
                  }}
                />
              </label>
          ...
          <div className="flex justify-end gap-4 pt-6 border-t dark:border-gray-800">
            <button
              type="submit"
              className="px-12 py-4 bg-emerald-600 hover:bg-emerald-700 text-white font-black rounded-2xl shadow-xl transition-all active:scale-95 uppercase tracking-wider"
            >
              Publish Product
            </button>
          </div>
