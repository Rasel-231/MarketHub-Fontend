...
import { useCreateOrderMutation } from "@/components/Redux/api/orderApi/orderApi";
import BreadCumb from "../../common/BreadCumb/BreadCumb";
import { toast } from "react-toastify";
import { useGetCartsQuery } from "@/components/Redux/api/cartApi/cartApi";
import { ShoppingCart, Loader2 } from "lucide-react";
import { ICart,IOrder,OrderFormData } from "@/types/types";
import { useCreatePaymentMutation } from "@/components/Redux/api/paymentApi/paymentApi";
...
const BillingDetails = () => {
  ...
  const onSubmit = async (data: OrderFormData) => {
    ...
    const orderPayload = {
      ...
      city: data.city,
      paymentMethod: data.paymentMethod === "online" ? "ONLINE" : "CASH_ON_DELIVERY",
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          ...
          <div className="space-y-6">
            <div className="flex flex-col gap-2">
              <label className="text-gray-500">Name<span className="text-red-500">*</span></label>
              <input {...register("name", { required: true })} placeholder="Name" className="p-3 bg-gray-100 rounded-sm outline-none focus:ring-1 focus:ring-red-500 w-full" />
            </div>
            <div className="flex flex-col gap-2">
              <label className="text-gray-500">Street Address<span className="text-red-500">*</span></label>
              <input {...register("address", { required: true })} placeholder="Address" className="p-3 bg-gray-100 rounded-sm outline-none focus:ring-1 focus:ring-red-500 w-full" />
            </div>
            <div className="flex flex-col gap-2">
              <label className="text-gray-500">Town/City<span className="text-red-500">*</span></label>
              <input {...register("city", { required: true })} placeholder="Town/City" className="p-3 bg-gray-100 rounded-sm outline-none focus:ring-1 focus:ring-red-500 w-full" />
            </div>
            <div className="flex flex-col gap-2">
              <label className="text-gray-500">Phone Number<span className="text-red-500">*</span></label>
              <input {...register("phone", { required: true })} placeholder="Phone Number" className="p-3 bg-gray-100 rounded-sm outline-none focus:ring-1 focus:ring-red-500 w-full" />
            </div>
            <div className="flex flex-col gap-2">
              <label className="text-gray-500">Email Address<span className="text-red-500">*</span></label>
              <input {...register("email", { required: true })} placeholder="E-mail" className="p-3 bg-gray-100 rounded-sm outline-none focus:ring-1 focus:ring-red-500 w-full" />
            </div>
          </div>
          ...
          <div className="lg:mt-0">
            <div className="space-y-4 mb-8 text-lg bg-gray-50 p-6 rounded-md">
              <div className="flex justify-between items-center border-b pb-4">
                <div className="flex items-center gap-4"><ShoppingCart /><p>Total Products</p></div>
                <p className="text-2xl font-bold">{carts.items.length}</p>
              </div>
              <div className="flex justify-between py-2 border-b"><p>Subtotal:</p><p>${carts.totalAmount.toFixed(2)}</p></div>
              <div className="flex justify-between py-2 font-bold text-xl"><p>Total:</p><p>${carts.totalAmount.toFixed(2)}</p></div>
            </div>

            <div className="space-y-4 mb-8">
              <div className="flex items-center gap-3">
                <input {...register("paymentMethod")} type="radio" id="online" value="online" className="accent-black w-5 h-5" />
                <label htmlFor="online" className="cursor-pointer font-medium">Online Payment (SSLCommerz)</label>
              </div>
              <div className="flex items-center gap-3">
                <input {...register("paymentMethod")} type="radio" id="cod" value="cod" className="accent-black w-5 h-5" />
